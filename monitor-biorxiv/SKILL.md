...
Do not wait for answers before returning the first daily scan. If the user does not specify a filter, use the default method/algorithm-centered filter: algorithms, data structures, compression, graph/tree methods, set systems, combinatorics, computational and statistical method development, scalable bioinformatics software and data systems, structural-similarity search and molecular retrieval, reusable pipelines/workflows, regenerable reference databases/resources, genomic indexing/retrieval, synteny/orthogroup methods, spatial-omics methods, and methodological harmonization/benchmarking. Use an unfiltered bioRxiv feed only when the user explicitly asks for it. If the user asks to document preferences in `SKILL.md`, summarize the exact additions and update it only after direct confirmation; explain that project-skill changes affect future uses of this skill.
...
For each paper, retain the **complete, unabridged title**, authors, posted date, DOI/link, and one-sentence relevance note tied to the chat-derived profile. Never truncate a title or replace any part of it with an ellipsis. Render **one paper per bullet/list line**, with a blank line between entries; never run multiple papers together in a paragraph or a single long notification line. State the filters and date range at the top. Group the complete list into `High relevance`, `Worth a look`, and `Watchlist` only when useful; do not omit lower-priority matches.
...
Treat this as a **metadata-based estimate**, not an official bioRxiv category and never as a claim that every computational-biology paper was found. Mark a paper as computational biology only when its title or abstract indicates that computation is a central contribution, such as bioinformatics, algorithm or statistical-method development, feature selection, scalable software or data systems, a reusable pipeline, retrieval/indexing, synteny/orthogroup analysis, genome reconstruction, spatial-omics methods, a regenerable reference database/resource, or a computational method/model.

Include a resource when it provides a reusable, reproducible, or regenerable computational asset rather than merely depositing a one-off dataset. Include a harmonization, preprocessing, or benchmarking paper when it establishes a broadly reusable comparison or decision framework; rank it below new algorithms, systems, databases, and pipelines. Exclude a wet-lab workflow unless its central output is reusable computational software or analysis infrastructure.

Treat a distinctive tool name followed by a colon—such as `LEN-Seek: …`, `scFair: …`, or `SecretTarget: …`—as a positive **named-method signal** when the remainder of the title or abstract establishes a computational method, search/retrieval system, software tool, pipeline, database, or benchmark. Do not treat the colon format alone as sufficient evidence.
